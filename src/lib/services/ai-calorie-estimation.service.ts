...
Only respond with valid JSON, no additional text.`;
    ...
    try {
      ...
      const fallbackCalories = this.getFallbackEstimation(
      ...
      return {
        ...
        nutrition: {
          ...
          carbs: Math.round((fallbackCalories * 0.5) / 4), // ~50% carbs
  ...
  private getFallbackEstimation(foodDescription: string): number {
    ...
    if (highCalorieKeywords.some((keyword) => description.includes(keyword))) {
      ...
      mediumCalorieKeywords.some((keyword) => description.includes(keyword))
    ) {
      baseCalories = 300;
    } else if (
      lowCalorieKeywords.some((keyword) => description.includes(keyword))
    ) {
      baseCalories = 150;
    }
